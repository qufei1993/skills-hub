...
- **Accurate Skill sync status**: Skill details, cards, and dashboard summaries now distinguish healthy, partial, failed, and not-yet-synced states. Failed target attempts are persisted with their error details, and clicking a failed tool retries the sync instead of attempting to disable it.
