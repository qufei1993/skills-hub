...
### 修复
- **记录还在、内容已经丢失的 Skill**：中央仓库目录已经消失的 Skill 不再被报告为正常。此前的状态检查对没有外部来源的本地 Skill 会提前返回，导致目录缺失时仍显示正常、原因也记为未知；现在每个本地 Skill 都会检查中央目录，并在报告中以 `centralMissing` 说明原因。
- **从中央仓库找回 Skill**：引导扫描新增中央仓库作为发现来源，磁盘上仍在、但已不在库中的 Skill 会按其目录名列出以供认领，与在工具目录中发现的 Skill 一致。
- **认领已有记录的目录**：当目录名与一条内容已丢失的记录同名时，导入已有 Skill 会为同一个 Skill 再建一条记录；现在改为修复原记录，指向实际存在的目录，并保留其 id、标签、启用状态和工具目标。
- **删除内容已经丢失的 Skill**：此前的删除要求来源目录必须是目录，导致这种空壳记录完全无法从库中移除。现在可以直接删除，只在指向真实内容时清理工具链接，也不会在回收站留下空条目。
...
- **Kimi Code CLI 路径与图标**：将全局、项目和检测目录修正为 `.kimi-code`，支持 `KIMI_CODE_HOME` 并使用官方产品图标，同时保留旧路径中的已有内容（修复 [#122](https://github.com/qufei1993/skills-hub/issues/122)，[PR #126](https://github.com/qufei1993/skills-hub/pull/126)）。

### 安全
- **凭据保护与可移植同步边界**：HTTPS Token 保存在操作系统凭据存储中，SSH 使用系统 Agent；公开读取不访问凭据，公开上传需要确认，私钥内容会被拒绝。工具目标、项目路径、设置、缓存和凭据始终留在本机（[PR #127](https://github.com/qufei1993/skills-hub/pull/127)、[PR #134](https://github.com/qufei1993/skills-hub/pull/134)）。
