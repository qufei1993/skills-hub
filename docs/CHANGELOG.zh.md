...
### 修复
- **自动更新时出现的卡死窗口**：计划任务执行的后台更新，以及更新菜单里的「立即更新」，都不再弹出窗口。后台运行会在应用构建前被识别，并跳过创建 `tauri.conf.json` 中声明的窗口——这是唯一能阻止打包窗口出现的位置；「立即更新」也改为在当前运行的应用内执行，不再请求操作系统计划程序启动第二个实例。该操作同样不再重新注册计划任务，因此关闭自动更新后不会在下次手动更新时被恢复。等待并发 SQLite 写入的连接现在会等待，而不是立即以 `SQLITE_BUSY` 失败（修复 [#152](https://github.com/qufei1993/skills-hub/issues/152)）。
