...
### 新增
- **单向库同步**：设备同步现在可以「从仓库拉取到本地」或「推送本地到仓库」，无需走三方合并。两个方向都只新增与更新，因此单向操作不会删除任何内容。

### 修复
- **设备同步会删除它读不到的内容**：中央目录已丢失的 Skill 在导出库时会被跳过，因此不会进入本地清单，而三方合并会把它理解为「本机已删除」——于是删除仓库中的副本并推送该删除，远端删除随后又移除了本地记录。内容已丢失的库因此可能反过来清空仓库。现在无法导出的 Skill 会被告知合并流程，改为从仓库恢复而不是删除；跳过行为也会写入日志，便于排查。
