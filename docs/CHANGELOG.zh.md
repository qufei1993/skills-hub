...
### 修复
- **安全迁移与移除 Skills**：修改 Skills 中心存储位置时，会拒绝与工具同步目录、本地原始源目录或当前存储目录树重叠的路径；已有 Skills 需要明确确认后才会迁移，底层同步也会独立拦截源目录与目标目录重叠的操作。用户执行移除时，真实文件和目录会进入系统回收站，本地原始源目录会保留（[#123](https://github.com/qufei1993/skills-hub/issues/123)）。
