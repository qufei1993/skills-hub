...
const DeviceSyncPage = ({
  ...
  const load = useCallback(async ({ refreshRepositories = true }: LoadOptions = {}) => {
    if (!isTauri) return
        ...
        {experience.mode === 'dashboard' ? (
          <section className={`device-sync-status-strip ${experience.syncState}`}>
            <div className="device-sync-status-copy">
              ...
              <span><strong>{t(`deviceSync.visualState.${experience.syncState}`, { count: conflicts.length })}</strong><small>{t('deviceSync.lastSyncValue', { value: status?.last_run_at ? new Date(status.last_run_at).toLocaleString() : t('deviceSync.never') })}</small></span>
            </div>
            <div className="device-sync-route" aria-label={t('deviceSync.routeLabel')}>
              <div className="device-sync-route-node ready"><span><Laptop size={18} /></span><strong>{currentDevice?.alias || currentDevice?.name || t('deviceSync.thisDevice')}</strong><small>{t('deviceSync.currentDevice')}</small></div>
              <i className={experience.syncState === 'healthy' ? 'complete' : ''} />
              <div className="device-sync-route-node ready"><span><FolderGit2 size={18} /></span><strong>{providerName}</strong><small>{repositoryDisplayName}</small></div>
              <i className={experience.syncState === 'healthy' ? 'complete' : ''} />
              <button className={`device-sync-route-node device-sync-device-summary ${otherDeviceSummary.pendingCount ? 'attention' : 'ready'}`} type="button" onClick={() => setActivityTab('devices')}><span><Monitor size={18} /></span><strong>{otherDeviceSummary.count ? t('deviceSync.otherDeviceCount', { count: otherDeviceSummary.count }) : t('deviceSync.noOtherDevices')}</strong><small>{otherDeviceSummary.count ? (otherDeviceSummary.pendingCount ? t('deviceSync.pendingDevices', { count: otherDeviceSummary.pendingCount }) : t('deviceSync.otherDevicesUpToDate')) : t('deviceSync.noOtherDevicesHelp')}</small></button>
            </div>
            <div className="device-sync-status-actions"><button className="btn btn-secondary" type="button" disabled={!controls.canCheck} onClick={check}>{busy === 'check' ? <LoaderCircle className="spin" size={15} /> : <RefreshCw size={15} />}{t('deviceSync.check')}</button><button className="btn btn-primary" type="button" disabled={!controls.canSync || conflicts.length > 0} onClick={sync}>{synchronizationInProgress ? <LoaderCircle className="spin" size={15} /> : <Cloud size={15} />}{conflicts.length ? t('deviceSync.waitingForConflicts') : t('deviceSync.syncNow')}</button></div>
          </section>
        ) : (
          ...
          <section className="device-sync-panel device-sync-setup-panel">
            ...
            <div className="device-sync-setup-content">
              ...
              {authorized ? <div className="device-sync-repository-step"><div className="device-sync-repository-step-head"><span><strong>{t('deviceSync.chooseRepository')}</strong><small>{t('deviceSync.repositoryStorageHelp')}</small></span><div className="device-sync-repository-actions"><button className="btn btn-secondary" type="button" disabled={working || repositoryLoadState === 'loading'} onClick={retryRepositories}>{t(form.remoteUrl ? 'deviceSync.refreshRepositories' : 'deviceSync.loadRepositories')}</button><button className="btn btn-secondary" type="button" disabled={!controls.canCreateRepository} onClick={createRepository}>{busy === 'create' ? <LoaderCircle className="spin" size={15} /> : null}{t('deviceSync.createPrivateRepository')}</button></div></div>
                {repositoryLoadState === 'idle' && form.remoteUrl ? <label className="device-sync-repository-choice selected"><input type="radio" checked readOnly /><span><strong>{repositoryDisplayName}</strong><small>{t('deviceSync.currentRepository')}</small></span><LockKeyhole size={15} /></label> : null}
                {repositoryLoadState === 'loading' ? <div className="device-sync-repository-feedback loading" role="status" aria-live="polite"><LoaderCircle className="spin" size={16} /><span>{t('deviceSync.loadingRepositories')}</span></div> : null}
                {['error', 'timeout', 'credential-error'].includes(repositoryLoadState) ? <div className="device-sync-repository-feedback error" role="alert"><AlertTriangle size={16} /><span>{repositoryErrorMessage}</span><button type="button" onClick={retryRepositories}>{t('deviceSync.retry')}</button></div> : null}
                {repositoryLoadState === 'loaded' && repositories.length ? <div className="device-sync-repository-choices">{repositories.map((repository) => <label key={repository.clone_url} className={`device-sync-repository-choice${form.remoteUrl === repository.clone_url ? ' selected' : ''}`}><input type="radio" name="device-sync-repository" value={repository.clone_url} checked={form.remoteUrl === repository.clone_url} onChange={(event) => setForm({ ...form, remoteUrl: event.target.value })} /><span><strong>{repository.name}</strong>{repository.name === 'skills-hub-sync' ? <small>{t('deviceSync.recommended')}</small> : null}</span><LockKeyhole size={15} /></label>)}</div> : null}
                {repositoryLoadState === 'loaded' && !repositories.length ? <p className="device-sync-repository-empty">{t('deviceSync.noPrivateRepositories')}</p> : null}
              </div> : null}
          ...
          <aside className="device-sync-settings-drawer" role="dialog" aria-modal="true" aria-labelledby="device-sync-settings-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="device-sync-drawer-content">
              <section><h3>{t('deviceSync.connection')}</h3><div className="device-sync-settings-rows"><div><span><FolderGit2 size={16} />{t('deviceSync.provider')}</span><strong>{providerName}</strong></div><div><span><LogIn size={16} />{t('deviceSync.authorizedAccount')}</span><strong>{form.accountLogin || config.username || '—'}</strong><button type="button" disabled={working || !oauthAvailable} onClick={startAuthorization}>{t('deviceSync.reauthorize')}</button></div><div><span><FolderGit2 size={16} />{t('deviceSync.repository')}</span><strong>{repositoryDisplayName}</strong><button type="button" aria-expanded={repositoryPickerOpen} aria-controls="connected-device-sync-repositories" disabled={working} onClick={toggleRepositoryPicker}>{t(repositoryPickerOpen ? 'deviceSync.collapseRepositories' : 'deviceSync.changeRepository')}{repositoryPickerOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button></div></div>
                {repositoryPickerOpen ? <div id="connected-device-sync-repositories" className="device-sync-repository-picker">{repositoryLoadState === 'loading' ? <div className="device-sync-repository-feedback loading"><LoaderCircle className="spin" size={16} />{t('deviceSync.loadingRepositories')}</div> : null}{repositoryLoadState === 'loaded' && repositories.length ? <div className="device-sync-inline-repositories">{repositories.map((repository) => <label key={repository.clone_url}><input type="radio" name="connected-device-sync-repository" value={repository.clone_url} checked={form.remoteUrl === repository.clone_url} onChange={(event) => setForm({ ...form, remoteUrl: event.target.value })} />{repository.name}</label>)}</div> : null}{repositoryLoadState === 'loaded' && !repositories.length ? <p className="device-sync-repository-empty">{t('deviceSync.noPrivateRepositories')}</p> : null}{['error', 'timeout', 'credential-error'].includes(repositoryLoadState) ? <div className="device-sync-repository-feedback error"><AlertTriangle size={16} />{repositoryErrorMessage}<button type="button" onClick={retryRepositories}>{t('deviceSync.retry')}</button></div> : null}</div> : null}
              </section>
              <section><h3>{t('deviceSync.automation')}</h3><div className="device-sync-drawer-options"><label><span><strong>{t('deviceSync.autoCheck')}</strong><small>{t('deviceSync.autoCheckHelp')}</small></span><input type="checkbox" checked={form.autoCheck} onChange={(event) => setForm({ ...form, autoCheck: event.target.checked })} /></label><label><span><strong>{t('deviceSync.autoSync')}</strong><small>{t('deviceSync.autoSyncHelp')}</small></span><input type="checkbox" checked={form.autoSync} onChange={(event) => setForm({ ...form, autoSync: event.target.checked })} /></label></div></section>
